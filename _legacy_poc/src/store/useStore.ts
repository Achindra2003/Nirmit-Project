...
interface StoreState {
  ...
  // ── Journey state (single IntakePayload replaces old ContextAnswers + VibeOption + Act1HandoffPayload) ──
  intakePayload: IntakePayload | null;
  setIntakePayload: (p: IntakePayload) => void;
  generatedLayouts: LayoutResult[];
  setGeneratedLayouts: (layouts: LayoutResult[]) => void;
  generatedRankings: AIRankedLayout[];
  setGeneratedRankings: (rankings: AIRankedLayout[]) => void;
  generatedRenders: Map<number, RoomRenderResult[]>;
  setGeneratedRenders: (renders: Map<number, RoomRenderResult[]>) => void;
...
export const useStore = create<StoreState>()(
  ...
  // ── Journey defaults ──
  intakePayload: null,
  setIntakePayload: (p) => set({ intakePayload: p }),
  generatedLayouts: [],
  setGeneratedLayouts: (layouts) => set({ generatedLayouts: layouts }),
  generatedRankings: [],
  setGeneratedRankings: (rankings) => set({ generatedRankings: rankings }),
  generatedRenders: new Map(),
  ...
  {
    name: 'nirmit-design-storage',
      storage: createJSONStorage(() => localStorage),
      partialize: (state) => ({
        items: state.items,
        room: state.room,
        materials: state.materials,
        intakePayload: state.intakePayload,
        generatedLayouts: state.generatedLayouts,
        generatedRankings: state.generatedRankings,
        selectedLayoutId: state.selectedLayoutId,
        doorWindows: state.doorWindows,
      }),
    }
  )
);

// ── Shallow comparison selectors for frequently accessed state slices ──

/** Select items with shallow comparison — avoids re-renders when items array reference is stable */
export function useItems() {
  return useStore(useShallow((s) => s.items));
}

/** Select room config with shallow comparison */
export function useRoom() {
  return useStore(useShallow((s) => s.room));
}

/** Select materials with shallow comparison */
export function useMaterials() {
  return useStore(useShallow((s) => s.materials));
}

/** Select advisories with shallow comparison */
export function useAdvisories() {
  return useStore(useShallow((s) => s.advisories));
}

/** Select active item ID */
export function useActiveItemId() {
  return useStore((s) => s.activeItemId);
}