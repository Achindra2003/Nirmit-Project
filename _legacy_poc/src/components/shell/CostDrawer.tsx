    ...
    <>
      ...
      >
        ...
        <div className="space-y-3 font-ui text-sm text-[var(--n-600)]">
          ...
          {/* GST Breakdown */}
          <div className="mt-3 rounded-xl border border-[var(--n-200)] bg-white p-3">
            <p className="font-ui text-[10px] uppercase tracking-[0.12em] text-[var(--n-400)] mb-2">GST Breakdown</p>
            <div className="flex items-center justify-between text-xs text-[var(--n-600)]">
              <span>CGST @9%</span>
              <span>{formatINR(summary.taxBreakdown.cgst)}</span>
            </div>
            <div className="flex items-center justify-between text-xs text-[var(--n-600)]">
              <span>SGST @9%</span>
              <span>{formatINR(summary.taxBreakdown.sgst)}</span>
            </div>
            <div className="mt-1 flex items-center justify-between border-t border-[var(--n-200)] pt-1 text-sm font-medium text-[var(--brand)]">
              <span>Total GST</span>
              <span>{formatINR(summary.taxBreakdown.totalTax)}</span>
            </div>
          </div>

          <div className="my-2 border-t-2 border-[var(--brand)]" />
          <div className="flex items-center justify-between font-display text-xl text-[var(--brand)]">
            <span>Grand Total (Incl. Tax)</span>
            <span>
              {formatINR(summary.totalLowWithTax)} - {formatINR(summary.totalHighWithTax)}
            </span>
          </div>
        </div>
        ...
        <div className="mt-6 space-y-3 rounded-xl border border-[var(--n-200)] bg-[var(--n-50)] p-4">
          ...
          <p className="font-ui text-xs text-[var(--n-600)]">
            GST: 18% on furniture, 28% on luxury items (marble, genuine leather).
          </p>
