...
import { CONTEXT_CITIES, MUST_HAVE_OPTIONS, type BudgetTier, type ContextAnswers } from '../types/journey';
        ...
        return (
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              {(['budget', 'mid-range', 'premium', 'luxury'] as BudgetTier[]).map((budgetTier) => {
                const selected = value.budget === budgetTier;
                const info: { range: string; desc: string } = {
                  'budget': { range: '₹50K – ₹1.5L', desc: 'Strict constraints. Smart modulars.' },
                  'mid-range': { range: '₹1.5L – ₹4L', desc: 'Balanced finishes. Good longevity.' },
                  'premium': { range: '₹4L – ₹10L', desc: 'Bespoke materials. High luxury.' },
                  'luxury': { range: '₹10L+', desc: 'Premium bespoke. Imported pieces.' }
                }[budgetTier] ?? { range: '', desc: '' };
                return (
                  <button
                    key={budgetTier}
                    type="button"
